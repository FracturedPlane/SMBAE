...
class CACLA2(AlgorithmInterface):
    """
        This version does not use a target network
    
    """
    
    def __init__(self, model, n_in, n_out, state_bounds, action_bounds, reward_bound, settings_):

        super(CACLA2,self).__init__(model, n_in, n_out, state_bounds, action_bounds, reward_bound, settings_)
        ...
        self._q_valsTargetNextState = lasagne.layers.get_output(self._model.getCriticNetwork(), self._model.getResultStateSymbolicVariable())
    ...
    def setData(self, states, actions, rewards, result_states, fallen):
        ...
        targets__ = self._get_target()[0]
        # print ("Targets: ", targets__)
        self._target_shared.set_value(targets__)
        ## Compute targets here
        """
    ...
    def trainActor(self, states, actions, rewards, result_states, falls):
        ...
        if (len(tmp_actions) > 0):
            ## Hack because of dimension mismatch between target and value function
