...
class ParticleGame(object):
    ...
    def initEpoch(self):
        ...
        """
        # new_loc = (np.random.rand(self._state_length) - 0.5 ) * (self._state_bounds[1][0] - self._state_bounds[0][0])
        new_loc = (np.random.rand(self._state_length) - 0.5) * (8 + 8)
        self._agent = new_loc
        # print ("Agent location: ", self._agent)
        new_loc = (np.random.rand(self._state_length) - 0.5) * (8 + 8)
        self._target = new_loc
        # print ("Target position: ", self._target)
    ...
    def reward(self, tmp_loc=None):
        ...
        if d < 0.1:
            ...
            # print ("Reward: ", reward)
            return reward
        reward = -d/((self._state_bounds[1][0]- self._state_bounds[0][0]))
        # print ("Reward: ", reward) 
        return reward
