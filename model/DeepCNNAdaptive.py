...
class DeepCNNAdaptive(ModelInterface):
    
    def __init__(self, n_in, n_out, state_bounds, action_bounds, reward_bound, settings_):

        super(DeepCNNAdaptive,self).__init__(n_in, n_out, state_bounds, action_bounds, reward_bound, settings_)
        ...
        for i in range(len(layer_sizes)):
            # [16,[8]]
            # [num_filters, [filter size, filter size, ...]]
            network = lasagne.layers.Conv1DLayer(
                network, num_filters=layer_sizes[i][0], filter_size=layer_sizes[i][1][0],
